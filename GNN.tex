...
There are many different architectures for GNNs in the literature and the goal of this paper is to compare these architectures in terms of expressiveness. We will start by giving a very general definition of a GNN and afterwards, we will discuss how particular architectures are captured by this definition.
...
Consider a more abstract presentation of~\eqref{eq:GNNs}.
We follow~\cite{grohewl,DBLP:conf/iclr/XuHLJ19} in which combination and aggregation functions for each layer $t$ are considered, denoted by
$f_{\textsl{comb}}^{(t)}$ and
$f_{\textsl{agg}}^{(t)}$, respectively. Then the layers are define by updates as follows. For each vertex $v\in V$ and layer $t$:
\begin{equation}
