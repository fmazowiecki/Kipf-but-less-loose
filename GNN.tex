...
Consider two architectures $\architecture$ and $\architecture'$. We say that $\architecture$ is bounded by $\architecture'$ if for every $f \in \architecture$ with the layer sequence $f^{(0)}, \ldots f^{(m)}$ there exists constants $a,b \in \Nb$ and $g \in \architecture'$ with the layer sequence $g^{(0)}, \ldots g^{(am + b)}$ such that $g^{(ai + b)} \sqsubseteq f^{(i)}$ for all $i = 0,\ldots,m$.
\end{definition}

Before we continue we present an example motivating the formulation of Definition~\ref{def:comparing}. It might seem natural to remove from the definition the two constants $a,b \in \Nb$.
